...
@section('content')
    <main class="md:columns-2 container mt-16 ">
        ...
        @include('components.fertilizer-recommendation-form')
    </main>
@endsection
...
@section('scripts')
    <script>
        ...
        document.getElementById('product-form').addEventListener('submit', async function(event) {
            ...
            try {
                const response = await axios.post('http://127.0.0.1:5000/fertrec', dataToSend);
                alert('we recommend using ' + response.data.recommendation);
            } catch (error) {
